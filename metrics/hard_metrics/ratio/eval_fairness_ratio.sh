#!/bin/bash
#SBATCH --job-name=eval_fairness_ratio
#SBATCH --output=logs/eval_fairness_ratio_%j.out
#SBATCH --error=logs/eval_fairness_ratio_%j.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:1
...
# =============================================================================
# Document-level ratio fairness evaluation across models and data types.
# Runs fairness_doc_level.py for each (data_type, model) pair.
# Usage: sbatch eval_fairness_ratio.sh
# =============================================================================
...
LOG_FILE="logs/eval_fairness_ratio_${SLURM_JOB_ID}.log"
...
for data_type in "${DATA_TYPES[@]}"; do
  for model_path in "${MODELS[@]}"; do
    ...
    start=$(date +%s)
    if python fairness_doc_level.py "$SUMMARY_FILE" --output "$OUTPUT_FILE" --no-cache; then
