#!/bin/bash
#SBATCH --job-name=eval_entity_diversity
#SBATCH --output=logs/eval_entity_diversity_%j.out
#SBATCH --error=logs/eval_entity_diversity_%j.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:1
...
# =============================================================================
# Entity diversity evaluation across models and data types.
# Runs entity_diversity.py for each (data_type, model) pair.
# Usage: sbatch eval_entity_diversity.sh
# =============================================================================
...
EVAL_SUBPATH="soft/entity_diversity"
...
LOG_FILE="logs/eval_entity_diversity_${SLURM_JOB_ID}.log"
...
# Ensure spaCy model is available (no-op if already installed)
...
for data_type in "${DATA_TYPES[@]}"; do
  for model_path in "${MODELS[@]}"; do
    ...
    start=$(date +%s)
    if python entity_diversity.py --input "$SUMMARY_FILE" --output "$OUTPUT_FILE"; then
