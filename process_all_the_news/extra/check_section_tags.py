...
def filter_entertainment_only_events(text_content):
    """
    Filters out events that contain ONLY entertainment-related articles.
    Events with a mix of entertainment and non-entertainment articles are kept.
    """
    # Define entertainment-related sections to recognize based on the comprehensive list
    entertainment_sections = [
        # Core entertainment categories
        'music', 'tv', 'entertainment', 'style', 'celebrity', 'movies', 'hollywood', 
        'showbiz', 'shows', 'Entertainment', 'TV', 'Movies',
        
        # Specific entertainment subcategories
        'Celebrity News', 'Entertainment News', 'Music by VICE', 'Noisey', 
        'Couples', 'royals', 'Departed', 'Scandal', 'Reality', 'Late Night',
        'Streaming', 'Feud', 'Oscars', 'Emmys', 'people-tv-ew', 'Celebrity Chefs',
        
        # Individual celebrities/shows
        'Scarlett Johansson', 'Demi Lovato', 'Meghan Markle', 'British Royals',
        
        # Entertainment adjacent categories
        'Style + Beauty', 'food', 'Food-Drink', 'Fast Food', 'Wine', 'Restaurants'
    ]
    ...
    try:
        ...
        for event_id, event_data in data.items():
            ...
            if not articles:
                events_to_keep.append(event_id)  # Keep events with no articles (benefit of doubt)
                continue
                
            # Check if all articles have entertainment sections
            all_entertainment = True
            for article in articles:
                section = article.get('section', '')
                if section and isinstance(section, str) and section.lower() not in [s.lower() for s in entertainment_sections]:
                    all_entertainment = False
                    break
            
            if all_entertainment and len(articles) > 0:
    ...
    # Find all events in the content using a more precise pattern
    event_pattern = r'"(MERGED_EVENT_[^"]*|EVENT_[^"]*)":\s*\{[^{]*?"articles":\s*\[(.*?)\]\s*\}'
    events = re.findall(event_pattern, text_content, re.DOTALL | re.IGNORECASE)
    
    # Process each event to check if it contains non-entertainment articles
    events_to_keep = []
    events_to_filter = []
    
    for event_id, articles_section in events:
        # Find all sections in this event
        section_matches = re.findall(r'"section":\s*"?([^",\n]*)"?', articles_section, re.IGNORECASE)
        
        # Check if all sections are entertainment-related
        has_non_entertainment = False
        for section in section_matches:
            section_lower = section.lower()
            if section_lower and section_lower != "nan" and section_lower not in [s.lower() for s in entertainment_sections]:
                has_non_entertainment = True
                break
        
        if has_non_entertainment or not section_matches:  # Keep if has non-entertainment OR no sections found
            events_to_keep.append(event_id)
        else:
            events_to_filter.append(event_id)
...
def save_filtered_events(text_content, event_filter_results, output_file):
    ...
    Args:
        ...
        event_filter_results: Results from the filter_entertainment_only_events function
    ...
    if event_filter_results.get('parsed_data'):
        ...
        if kept_events:
            ...
            print(f"The file contains {len(kept_events)} non-entertainment-only events")
            return True
        else:
    ...
    try:
        ...
        if event_blocks:
            ...
            if kept_count > 0:
                ...
                print(f"The file contains {kept_count} non-entertainment-only events")
                return True
            else:
...
def process_file(input_file, output_file, analysis_only=False):
    """
    Process a single file to filter out entertainment-only events.
    ...
    try:
        ...
        # Filter out events containing only entertainment content
        print("\n\nFILTERING EVENTS WITH ONLY ENTERTAINMENT CONTENT:")
        event_filter_results = filter_entertainment_only_events(text_content)
        print(f"Total events: {event_filter_results['total_events']}")
        print(f"Events with only entertainment articles (filtered out): {event_filter_results['events_to_filter']}")
        print(f"Events to keep (have non-entertainment articles): {event_filter_results['events_to_keep']}")
...
def process_directory(input_dir, output_dir, analysis_only=False):
    ...
    for input_file in json_files:
        ...
        output_file = os.path.join(output_dir, f"{os.path.splitext(file_name)[0]}_wo_entertainment.json")
...
def main():
    """
    Main function to process command line arguments and analyze sections.
    """
    # Set up argument parser
    parser = argparse.ArgumentParser(description='Analyze and filter news article sections')
    ...
    if args.batch or os.path.isdir(args.input):
        ...
        if args.output is None:
            output_dir = os.path.join(os.path.dirname(input_dir), "wo_entertainment")
        else:
        ...
        if args.output is None:
            ...
            file_name = os.path.basename(input_file)
            output_file = os.path.join(base_dir, "wo_entertainment", f"{os.path.splitext(file_name)[0]}_wo_entertainment.json")
        else:
